...
def summarize_comm_flocking(rollout, max_steps: int) -> dict[str, float]:
    done = rollout["next", "done"].squeeze(-1).bool()
    success = done.any(dim=1)
    episode_len = first_done_lengths(done, max_steps)
    valid = valid_step_mask(episode_len, done.shape[1])
    ...
    info_prefix = ("next", "agents", "info")
    if has_key(rollout, (*info_prefix, "agent_collision_rew")):
        collision_rew = rollout[(*info_prefix, "agent_collision_rew")].squeeze(-1)
        episode_collision_rew = (collision_rew * valid.unsqueeze(-1)).sum(dim=1)
        metrics.update(
            stats("agent_collision_rew", episode_collision_rew.mean(dim=-1))
        )

    if has_key(rollout, (*info_prefix, "agent_distance_rew")):
        distance_rew = rollout[(*info_prefix, "agent_distance_rew")].squeeze(-1)
        episode_distance_rew = (distance_rew * valid.unsqueeze(-1)).sum(dim=1)
        metrics.update(
            stats("agent_distance_rew", episode_distance_rew.mean(dim=-1))
        )
...
def filter_metrics(
    ...
    group_keys = {
        ...
        "comm_flocking": {
            ...
            "success_rate": ("success_rate", "timeout_rate"),
            "episode_len": ("episode_len_",),
            "collision": ("agent_collision_rew_",),
            "distance": ("agent_distance_rew_",),
            "communication": ("mean_aoi", "mean_comm_mask"),
        },
    }
